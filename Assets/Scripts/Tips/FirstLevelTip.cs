...
public class FirstLevelTip : ObjectPool
{
    ...
    private void Update()
    {
        if(_player.PaintAmount < _player.Paint—onsumption && _isCurrentTargetWalls == true)
        {
            ChangeTargets(_buckets);
        }
        else if(_player.PaintAmount >= _player.Paint—onsumption && _isCurrentTargetWalls == false)
        {
